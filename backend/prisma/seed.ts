...
async function main() {
  ...
  // Admin user
  const hashedPassword = await bcrypt.hash(process.env.ADMIN_INITIAL_PASSWORD || 'admin123456', 12);
  await prisma.user.upsert({
    where: { email: process.env.ADMIN_EMAIL || 'sitrakaharinjaka@gmail.com' },
    update: {
      password: hashedPassword,
    },
    create: {
      email: process.env.ADMIN_EMAIL || 'sitrakaharinjaka@gmail.com',
      password: hashedPassword,
